...
function iniciarApp(){
    ...
    if(selectCategorias){ //como en la pagina de favoritos nos da error diciendo que no hay ningun selectCategorias, le ponemos IF
        selectCategorias.addEventListener('change', seleccionarCategoria);
        obtenerCategorias();
    }

    const favoritosDiv = document.querySelector('.favoritos'); //(este solo se encuentra en el html de favoritos)
    if(favoritosDiv){ //si existe favoritosDiv ejecuta la siguiente funcion
        obtenerFavoritos();
    }
    ...
    function mostrarRecetas(recetas = []){
        ...
        recetas.forEach(receta =>{
            ...
            //Ahora bien, porque usamos el operador ??, esto es porque cuando llamamos a mostrarRecetas(favoritos), este es 
            //diferente al argumento que usamos con mostrarRecetas en el FECTH, tiene los datos de la API, entonces es diferente
            //al array que tenemos en Local Storage, buscaremos como podriamos acceder a esos valores, por eso usamos el ??
            ...
            recetaBtn.onclick = function(){
                seleccionarReceta(idMeal ?? receta.id); //al poner la function de esa manera, es como ponerlo como callback, estamos esperando
    ...
    function seleccionarReceta(id){
        ...
        fetch(url)
            .then(respuesta => respuesta.json())
            // .then(smn => console.log(smn.meals))
    ...
    function mostrarRecetaModal(receta){
        ...
        btnFavorito.onclick = function(){
            ...
            //Le agregamos un Objecto que se pasara a LocalStorage
            agregarFavorito({
                id: idMeal,
                title: strMeal,
                img: strMealThumb
    ...
    function existeStorage(id){
        const favoritos = JSON.parse(localStorage.getItem('favoritos')) ?? [];
        //puede ser que no exista ese getItem de favoritos,
        //entonces agregale un arreglo vacio
        //este operador se llama nullish coalescing, en caso de que la evaluacion del lado izquierdo marque null, aplique lo del
        //lado derecho

        //some itera sobre todos los elementos del arreglo y retorna true si alguno cumple con la condicion
        return favoritos.some(favorito =>{ //aqui volvimos a usar el return porque tenemos que retornar ese metodo a la funcion
            //existeStorage() que se usara mas arriba en el IF
            return favorito.id === id;//nos retorna un true o false
        })
    }
    ...
    function obtenerFavoritos(){ //puede ser que la primera vez que el usuario visite esa pagina, puede o no que tenga en el
        //local storage algun favorito, entonces ponemos lo siguiente
        const favoritos = JSON.parse(localStorage.getItem('favoritos')) ?? [];
        if(favoritos.length){ //si hay algo en los favoritos ejecuta el siguiente codigo
            mostrarRecetas(favoritos); //va a ser un arreglo el argumento y se comenzara a iterar en la funcion
            //La funcion de arriba a simple vista pensamos que al final de su codigo de la funcion el resultado.appendChild
            //va a imprimir en ambos documentos ya que ambos tienen el ID resultado, pero como esta funcion es exclusiva de
            //favoritos.html y la clase .favoritos SOLO IMPRIME EN FAVORITOS.HTML, SE PODRIA DECIR QUE AGARRA LA REFERENCIA Y SOLO
            //ACFECTA A FAVORITOS.HTML SIN TOMAR EN CUENTA A INDEX.HTML, es como una cadena que no tiene bifurcaciones.
            return
        }else{
            const noFavoritos = document.createElement('p');
            noFavoritos.textContent = 'No hay favoritos aun';
            noFavoritos.classList.add('fs-4', 'text-center', 'font-bold', 'mt-5');
            resultado.appendChild(noFavoritos); //resultado EXISTE EN AMBOS DOCUMENTOS, INDEX Y FAVORITOS
            //pero como esta funcion es exclusiva de favoritosDiv que tiene de clase .favoritos, esa clase solo existe en el
            //html de favoritos, no del index, es por eso que AQUI EN FAVORITOS SI SE IMPRIME Y EN INDEX NO

            //Para mas info de porque se imprime esto solo en favoritos.html y no index.html checar la prueba que yo hice 
            //en Prueba 2 Ventanas 
